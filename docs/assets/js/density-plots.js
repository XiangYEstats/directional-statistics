(function () {
  ...
  lab.querySelectorAll("[data-density-card]").forEach(card => {
    ...
    function update(state) {
      ...
      card.querySelector("[data-density-polar]").setAttribute("d", paths.polar);
      card.querySelector("[data-density-polar-area]").setAttribute("d", paths.polarArea);
      card.querySelector("[data-density-line]").setAttribute("d", paths.linear);
      card.querySelector("[data-density-area]").setAttribute("d", paths.area);
      card.querySelectorAll("[data-density-scale]").forEach(el => { el.textContent = format(result.scale); });
      card.querySelector("[data-density-kappa-output]").textContent = kappa.toFixed(1);
      card.querySelector("[data-density-location-output]").textContent = kind === "vm" ? angleLabel(locationValue) : locationValue.toFixed(2);
      ...
      card.querySelector("[data-density-note]").textContent = note;
      ...
      card.querySelectorAll("[data-density-description]").forEach(el => {
      ...
      card.querySelector("[data-density-status]").textContent = summary;
    }
    ...
    function schedule() {
      ...
      function animate(time) {
        const fraction = Math.min(1, Math.max(0, (time - firstTime) / 180));
